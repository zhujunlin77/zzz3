...
class COCO(data.Dataset):
    ...
    # __init__ 接收从主脚本传递过来的 opt 对象
    def __init__(self, opt, split):
        super(COCO, self).__init__()
        self.opt = opt  # 存储 opt 对象以访问路径等信息
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        scale_w = target_w / original_w if original_w > 0 else 0
        scale_h = target_h / original_h if original_h > 0 else 0
    ...
    # 评估相关函数 (convert_eval_format, run_eval, etc.) 保持不变
